 ...
 */
const pairs: [Model<Document>, string][] = [
    [User, 'Users'],
...
export const ADMIN_DB_REGISTRY: AdminDbRegistry[] = pairs.map(([model, label]) => ({
    key: model.collection.name,
    model,
    label
}));

const byKey = new Map<string, AdminDbRegistry>(ADMIN_DB_REGISTRY.map((e) => [e.key, e]));

export function getAdminModel(collectionKey: string): Model<Document> | null {
    const entry = byKey.get(collectionKey);
    return entry ? entry.model : null;
}

export function listAdminCollectionsMeta(): { key: string; label: string }[] {
